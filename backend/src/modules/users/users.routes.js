...
// Stored under students/{userId}/avatar/ — the same per-student root that
// onboarding documents use (students/{userId}/documents/), so every file a
// scholar owns lives under one structured, predictable folder.
...
router.post('/me/avatar', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const objectKey = `students/${userId}/avatar/${Date.now()}-${safe}`;
