...
export default function NotificationBell() {
  ...
  // This bell renders in BOTH AdminLayout and StudentLayout. GET /notifications
  // needs notifications:read, which several roles lack. Ensure permissions are
  // loaded (idempotent — a no-op if a layout already loaded them) and only fetch
  // when allowed, so no role 403s on every page. StudentLayout doesn't load
  // permStore itself, so the bell triggers it here.
  const canReadNotifications = usePermStore((s) => s.can('notifications', 'read'))
